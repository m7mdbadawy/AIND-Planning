...
class PlanningGraph():
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a,again=True) -> bool:
        '''
        Test a pair of actions for mutual exclusion, returning True if the
        ...
        for node_s in node_a1.effnodes:
            if node_s.is_pos and node_s.symbol in node_a1.action.precond_neg:
                return True
            if (not node_s.is_pos) and node_s.symbol in node_a2.action.precond_pos:
                return True
        #again variable is used to run the above loop with the other node effect nodes
        if again : return self.interference_mutex(node_a2,node_a1,False)
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            for i in range(len(self.s_levels)):
                if any(node_s.literal == g for node_s in self.s_levels[i]):
                    level_sum += i
                    break
        return level_sum
